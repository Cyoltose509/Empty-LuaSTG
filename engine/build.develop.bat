...
@setlocal
    ...
    :: x86
    @echo ============================ build Win32 ===========================
    cmake -S %SOURCE% -B %BUILD32% -G "Visual Studio 17 2022" -T v143 -A Win32 -D CPM_SOURCE_CACHE=%PACKAGE_CACHE% -D LUASTG_RESDIR=%CUSTOM%
    ::cmake --build %BUILD32% --target LuaSTG --config Debug
    cmake --build %BUILD32% --target LuaSTG --config Release --clean-first
    ::cmake --build %BUILD32% --target luajit --config Debug
