...
local ChargeMode = {}
local ChargeMode_COST = { 500, 1000, 2000, 5000, 10000, }
    ...
    for i = 1, 5 do
        ...
        if choose ~= 0 then
            ...
            elseif DEBUG then
                ...
                stagedata.chooseInitial[i] = 0
            end
        end
    end
end

function PlayerAchievementCheck(punit)
    local data = playerdata[punit.name]
    do
        if data.level >= 5 then
            mission_lib.GoMission(30)
        end
        if data.level >= 10 then
            mission_lib.GoMission(31)
        end
        local achievementGet = {
            Reimu = { 20, 35, 55, 88 },
            Marisa = { 21, 36, 56, 89 },
            Chiruno = { 124, 126, 128, 130 },
            Aya = { 125, 127, 129, 131 }
        }
        for i, lvl in ipairs({ 2, 10, 15, 20 }) do
            if achievementGet[punit.name] then
                if data.level >= lvl then
                    ext.achievement:get(achievementGet[punit.name][i])
                end
            end
        end
...
function SpellAchievementCheck(spunit)
    ...
    for i, lvl in ipairs({ 2, 7, 10 }) do
        if achievementGet[name] then
            if sp[1] >= lvl then
                ext.achievement:get(achievementGet[name][i])
            end
        end
    end
end
...
attributeselectmenu = stage.New("attr_select", false, true)

attributeselectmenu.general_buttonFrame = general_buttonFrame
attributeselectmenu.general_buttonRender = general_buttonRender
function attributeselectmenu:init()
    lastmenu = self
    self.top_bar = top_bar_Class(self, SceneClass[lstg.var.scene_id].title)
    ...
    self.start_button = {
        ...
        func = function()
            task.New(self, function()
                ...
                local ID = SceneClass[scene].inStageID
                local stg_name = { "Tutorial" .. scene, "Simple" .. ID, "Simple" .. ID, "Extra" .. ID }
                local id = stagedata.DiffSelection[DiffSelect.id]
                local cost = 0
                local function Start()
                    ...
                    mask_fader:Do("close")
                    task.Wait(15)
                    stage.group.Start(stage.groups[stg_name[id]])
                end
                for i = 1, 5 do
                    local choose = stagedata.chooseInitial[i]
                    if choose ~= 0 then
    ...
    task.New(self, function()
        ...
        if scoredata.guide_flag == 2 and scoredata.totalPearls > 0 then
            task.Wait(25)
            New(SimpleChoose, self, function()
                task.New(self, function()
                    scoredata.guide_flag = 3
                    mask_fader:Do("close")
                    task.Wait(15)
                ...
                scoredata.guide_flag = 3
            end, _t("guidance"), _t("consecrateGuide"), 200, 100)
        elseif scoredata.guide_flag == 4 and scoredata._total_mission > 0 then
            task.Wait(25)
            New(SimpleChoose, self, function()
                task.New(self, function()
                    scoredata.guide_flag = 5
                    mask_fader:Do("close")
                    task.Wait(15)
                ...
                scoredata.guide_flag = 5
            end, _t("guidance"), _t("exploreGuide"), 200, 100)
        else
            self.locked = false
        end
    end)
end
function attributeselectmenu:frame()
...
function attributeselectmenu:render()
...
--改的东西记得复制粘贴到attr_select2
function PlayerSelect:init()
    self.x, self.y = 480, 270
    self.func = function()
        self.locked = true
        attributeselectmenu.locked = true
    ...
    self.show_alpha = 1
    self.back = SceneClass[lstg.var.scene_id].tex
end
...
function DiffSelect:init()
    ...
    self.r = 58
    self.selected = false
    self.text = _t("selDiff")
    self.id = lstg.var.scene_id
    self.pos = stagedata.DiffSelection[self.id]
    self.diff_obj = {
        {
            title = "Tutorial",
            describe = _t("tutorialIntro"),
            colH = 135
        },
        {
            title = "Normal",
            describe = _t("normalIntro"),
            colH = 180
        },
        {
            title = "Ultra",
            describe = _t("ultraIntro"),
            colH = 270
        },
        {
            title = "Extra",
            describe = _t("extraIntro"),
            colH = 45
        },
    }
    self.unlock_extra = false
    for i = 2, 3 do
        self.unlock_extra = self.unlock_extra or (stagedata.stagePass[self.id][i] > 0)
    end
end
function DiffSelect:frame()
    stagedata.DiffSelection[self.id] = self.pos
    self.timer = self.timer + 1
    local mouse = ext.mouse
    local x, y = self.x, self.y
    ...
    if self.selected then
        if mouse._wheel ~= 0 then
            self.pos = sp:TweakValue(self.pos - sign(mouse._wheel), self.unlock_extra and 4 or 3, 1)
            self.index = self.index + 0.5
            PlaySound("select00")
        end
        if mouse:isUp(1) then
            for i = 1, 4 do
                ...
                if sp.math.PointBoundCheck(mouse.x, mouse.y, x1, x2, _y - _w / 2, _y + _w / 2) then
                    if not (not self.unlock_extra and i == 4) then
                        self.pos = i
                    end
                end
            end
    ...
    if menu:keyDown() then
        self.pos = sp:TweakValue(self.pos + 1, self.unlock_extra and 4 or 3, 1)
        self.index = self.index + 0.5
        PlaySound("select00")
    end
    if menu:keyUp() then
...
function DiffSelect:render()
    ...
    for i, p in ipairs(self.diff_obj) do
        ...
        ui:RenderText("pretty", p.title, x1 + 6, _y + _w / 2, 0.6,
                Color(index * 255, sp:HSVtoRGB(p.colH, 0.3, 1)), "left", "top")
        ui:RenderText("title", p.describe, x1 + 6, _y + 2, 0.6,
                Color(index * 255, sp:HSVtoRGB(p.colH, 0.1, 1)), "left", "top")
        local r, g, b = 200, 200, 200
        local text = ""
        if i == 4 and not self.unlock_extra then
            r, g, b = 200, 200, 200
            text = _t("unlocked")
        else
            if stagedata.stagePass[self.id][i] > 0 then
                r, g, b = sp:HSVtoRGB(90, 0.3, 1)
                if i == 1 then
                    text = _t("checked")
                else
                    local count = stagedata.stagePass[self.id][i]
                    text = _t("passCount"):format(count)
                end
            else
                r, g, b = sp:HSVtoRGB(0, 0.3, 1)
                text = _t("unpassed")
            end
        end
        SetImageState("menu_circle", "", 80 + index * 120, r, g, b)
        Render("menu_circle", x2 - 45, _y, 0, 28 / 192)
        ui:RenderText("pretty", text, x2 - 45, _y + 1, 0.37,
                Color(80 + index * 120, r, g, b), "centerpoint")
    end
    SetImageState("white", "", 255, 200 - k * 15, 200, 200 - k * 15)

    misc.RenderOutLine("white", x1, x2, y2, y1, 3, 0)
    for i = 1, 3 do
        local _y = y2 - (y2 - y1) / 4 * i
                ...
                if sp.math.PointBoundCheck(mouse.x, mouse.y, x1, x2, _y - _w / 2, _y + _w / 2) then
                    if stagedata.unlockInitialPos[i] then
                        ...
                        if count <= 5 then
                            ...
                            New(SimpleChoose, attributeselectmenu, function()
                                if scoredata.money >= money then
                                    PlaySound("extend")
                                    stagedata.unlockInitialPos[count] = true
                                    AddMoney(-money)
                                    attributeselectmenu.top_bar:SetShowAddMoney(-money)
                                    if count >= 2 then
...
function PlayerSelect_Submenu:Out()
    ...
    task.New(self, function()
        ...
        attributeselectmenu.locked = false
        for i = 1, 16 do
            self.alpha = 1 - i / 16
            task.Wait()
        end
