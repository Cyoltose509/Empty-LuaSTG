            ...
            if b_show_menubar then
                if imgui.ImGui.BeginMainMenuBar() then
                    ...
                    if imgui.ImGui.BeginMenu("Test") then
                        if imgui.ImGui.MenuItem("AddScore100000") then
                            lstg.var.score = lstg.var.score + 100000
                        end
                        imgui.ImGui.MenuItem(("GameTime:%d"):format(lstg.var.running_timer))
                        imgui.ImGui.MenuItem(("RealTime:%d"):format(lstg.var.real_running_timer))
                        imgui.ImGui.MenuItem(("ValidScore:%s"):format(lstg.var.valid_score and "true" or "false"))
                        imgui.ImGui.EndMenu()
                    end
